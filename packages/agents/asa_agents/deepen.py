...
只输出一个 JSON 对象,格式:
{"slides": [
  {"slide_id":"s1","layout_type":"title|section|bullet_evidence|two_column_table|formula_banner|figure_caption|figure_left|two_content|figure_grid|big_figure",
   "title":"<简洁一行,中文≤16字>",
...
规则:标题页/章节页 evidence_pages 可为 []、figure_id 为 null、table_refs 为 [];\
**若有补充数据表值得展示/作图,在相应页填 table_refs**(扩写阶段会拿到该表完整数据)。
- **严禁冗余**:不要出现两页讲同一个要点/同一张图/同一组数据,每页的 title 与 focus 必须覆盖不同内容,不要把一个结果拆成近乎重复的两页。
- **版式语义**:`title`/`section` 是**分隔页**,只有标题、不承载正文(无 bullet/表/图/图表/公式);只要本页有实质内容要讲,layout_type 必须用 `bullet_evidence`(或下列内容版式),不要错用 `section`。
- **版式词汇**(图为主的页 figure_id 必须取自可用图列表):`figure_caption`=图右文左、`figure_left`=图左文右(**相邻图文页交替使用,避免单调**);\
`big_figure`=整页大图(关键结果图值得放大时);`figure_grid`=2-4 张相关子图并排对比;`two_content`=两块内容并排;`two_column_table`=表左文右。
...
- **版式一致**:若 layout_type 是 `title`/`section`(分隔页),只给标题、不要输出任何内容 block;若本页确有正文要讲,本就不该是分隔页,请按 `bullet_evidence` 产出正文
