...
{"slide_id":"<沿用>","layout_type":"<沿用>","title":"<沿用或精炼>",
 "subtitle":"<内容页:一句话本页导读(≤25字,讲清本页核心发现/要点);封面:论文出处或英文原题;section/ending:一句导语;可为空>",
...
- 每条 bullet 至多一个 `**重点词或数字**`,**全页合计不超过 3 处**——强调泛滥等于没有强调
...
def _skeleton(
    ...
    prompt = (
        ...
        f"密度档位:**正文内容页** {prof['pages']} 页(封面/目录/章节分隔/致谢页**另算**,不占此预算——结构页不承载内容,绝不能用结构页凑页数)。\n现在产出骨架 JSON。"
    )
    if feedback:
