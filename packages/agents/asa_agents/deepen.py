...
- **总结收口**:结论/总结/展望合计**至多 1 个页面**(ending 之外),严禁"总结+主要贡献+未来方向"多页重复;展望内容必须来自论文的 discussion/limitations,**严禁论文没提的通用套话**(如"多模态融合""扩大数据集")。
- **目录与章节严格一致**:toc 列出的章节名必须与 `section` 分隔页一一对应(数量相同、名称一致)——目录承诺了就必须有对应分隔页;\n  **每个 `section` 分隔页紧贴其章节的内容页之前**,该章节的所有内容页都排在它后面、下一个 section 之前——不要把某章内容错放到别章的分隔页之后(页脚章节号据此自动生成,放错会串号)。
- **不规划单薄页**:某个要点若只有两三句可讲,并入相邻页,不要单列;`figure_grid` 仅在确有 2-4 张同主题子图时使用,只有一张图就用 figure_caption/figure_left。
- **子图使用规范(重要)**:图清单里标注「整图被拆成 N 张子图」的,**那 N 张都是同一张图的子图,没有单独的整图文件**——要展示该图就用 `figure_grid` 放入这些子图(全部或信息完整的几张);**绝不能挑其中带图注的那一张当整图用**(它只是一个子图);单用一张子图仅限它本身是大幅机制/流程图且自洽可读。标注「疑似子图,慎用」的不要用。
- **密集原图要放大**:图注/图例/轴文字密集的原图(多面板、坐标轴多)优先 `big_figure` 整页放大,或选其子图拆开放大——**不要塞进半栏缩小**(截图里的小字会不可读)。
...
- 顶部 y=60-100 放页标题(28-32px #333 粗体)与一句导读(14px #808080);内容区 y=130-670;对齐严格、留白充足\n- **底部页脚**(与其它页一致):在 y≈700 处放一行 12px #808080 文字「NN · 本页所属章节名」(NN 为两位章节号),保持全 deck 统一外壳,不要做成脱离模板的浮动卡片堆
- 文字行宽自己估算(中文≈字号 px/字),**绝不能溢出所在卡片/区域**;几何(网格/坐标轴/比例)要按数据计算,不要随手乱画
- **所有数字与结论必须来自给定证据,严禁编造**;证据里没有逐点数据时**禁止画具体数据点/散点**——改用区间带、箭头、结构示意等表达
- **画面上不得出现任何工具痕迹**:不要写 "Source"、"Evidence from page N"、"数据来源:第N页" 之类文字(溯源放 provenance 字段,不上画面)
- **观感与学术 deck 一致**:白底、低饱和、上面的调色板;不要网页风渐变/重阴影;中文一律 SimHei、西文与数字一律 Times New Roman,不得混用其它字体
...
{"slide_id":"<沿用>","layout_type":"<沿用>","title":"<沿用或精炼>",
 "subtitle":"<内容页:一句**结论式**导读(陈述本页发现,带关键数值,不是主题复述);**封面:作者等 · 单位/期刊 · 年份(从证据识别,如「Lin et al. · 期刊名 · 2025」)**;section/ending:一句导语;可为空>",
...
block 之一:
  ...
  {"type":"stat","items":[{"value":"r=0.938","label":"验证集相关系数","icon":"target"}]}(1-4 个**来自证据的**关键数值,适合成果/指标页;**前后改进可写成箭头形式**,如 value="0.89 → 0.93";严禁编造)
...
- **对照表(table)**:讲**方法对比/前人模型缺陷/多方案指标**时,优先输出 table 对照表(列=方法/模型,行=指标/特性,单元格填证据数值或✓/✗)——比 bullets 罗列高一个档次;表头列名要短。
...
- **图主导页文字克制**:figure_caption/figure_left/big_figure 页 bullets **至多 4 条、每条≤40字**——图是主角,长解读放进 speaker_notes,不要用文字墙把图挤小。
...
- **caption 写实并注明图号**:图注以原文图号开头——格式「**图N | 描述**」(N 从图注线索的 "Fig. N"/"图N" 识别;识别不出就只写描述);**caption 只能描述所选图的当前真实画面**——看不到的内容(整图其它子图的 (a)(b)、本图没画的数据)绝对不能写;\n  子图用格式「图N-{a/b/c…} | 描述」,描述只写画面可确认的内容,不确定就只保留图号、不要编造。
