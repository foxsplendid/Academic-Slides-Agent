...
规则:标题页/章节页 evidence_pages 可为 []、figure_id 为 null、table_refs 为 [];以图为主的页 layout_type 用 "figure_caption" 且 figure_id 必须取自可用图列表;\
**若有补充数据表值得展示/作图,在相应页填 table_refs**(扩写阶段会拿到该表完整数据)。
- **严禁冗余**:不要出现两页讲同一个要点/同一张图/同一组数据,每页的 title 与 focus 必须覆盖不同内容,不要把一个结果拆成近乎重复的两页。
- **版式语义**:`title`/`section` 是**分隔页**,只有标题、不承载正文(无 bullet/表/图/图表/公式);只要本页有实质内容要讲,layout_type 必须用 `bullet_evidence`(或 figure_caption/two_column_table 等内容版式),不要错用 `section`。
